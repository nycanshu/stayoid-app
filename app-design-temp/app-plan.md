...
> **Every feature in this doc is verified against the actual backend API (April 2026).**
> Features without a backing API endpoint are explicitly marked ❌ NOT YET SUPPORTED.
...
- Review records for a specific month
...
[Onboarding property setup: 4-step bottom sheet]
  Step 1: Property type → PG / Hostel / Apartment  ← exact backend enum values
...
─────────────────────────────────────────
  ...
  ─── Quick Actions ────────────────────
  [💳 Record]  [👤 Add Tenant]  [🏢 Add Property]
─────────────────────────────────────────
```
...
**Quick Actions row:**
- 3 pill buttons: Record Payment, Add Tenant, Add Property
- These are shortcuts; they duplicate the FAB for discoverability
- ❌ No Reports/Export API exists — do not add a Reports button
...
**Room grid design:**
- Each square represents one **Slot** (the actual occupancy unit in the API)
- Data source: `GET /api/slots/?property_id={id}` — flat endpoint, no need to traverse floors/units
- Each slot response includes: `slot_number`, `is_occupied`, `active_tenant.name`, `floor_number`, `unit_number`
- Grid grouped by `floor_number`, sorted ascending (Ground=0, First=1, etc.; Basement=-1/-2 shown at bottom)
- Card size: `w=80, h=80` on 390px screen = 4 per row
- Occupied: `bg-primary/15 border-primary/40 rounded-xl` + `active_tenant.name` first name in 11sp
- Vacant: `bg-muted border-border rounded-xl` + "Vacant" in 11sp muted
- Overdue: `bg-destructive/10 border-destructive/30` + red dot indicator (cross-reference with `tenants?unpaid=true`)
- Tap occupied slot → tenant detail (push screen using `active_tenant.slug`)
- Tap vacant slot → "Assign tenant" bottom sheet (POST /api/tenants/ with `slot_id`)
...
- **Month navigator** (← April 2026 →): tapping chevrons changes `month`/`year` query params → `GET /api/payments/?month=X&year=Y`
- **Grouped sections**: Two separate data sources merged in UI:
  - "UNPAID" section = `GET /api/tenants/?unpaid=true&month=X&year=Y` — tenants with no payment record this month
  - "PAID/PARTIAL/PENDING" section = `GET /api/payments/?month=X&year=Y` — actual payment records
  - Merge in the component: show unpaid tenants at top (amber rows), paid payments below
- **Swipe-to-action on UNPAID rows**: swipe right → "Record Payment" quick-action → opens Record Payment sheet pre-filled with that tenant (`POST /api/payments/` creates a new record)
- **Swipe-to-action on PAID rows**: swipe left → "Delete" (red) → `DELETE /api/payments/{id}/`
- **Payment row** shows avatar initials + tenant name + property/room + method badge + amount
...
```
  ...
  │  Amount  [₹ ____________]             │
  │  Month   [April 2026              ▼]  │
  │  Method  [UPI] [Cash] [Bank Transfer] [Cheque] [Other]  │
  │  Note    [Optional note...]           │
  │                                        │
  │        [Save Payment]                  │
  └────────────────────────────────────────┘
...
- Method is a segmented selector: UPI / Cash / Bank Transfer / Cheque / Other — exact backend enum values: `UPI`, `CASH`, `BANK_TRANSFER`, `CHEQUE`, `OTHER`
- Amount field: numeric keyboard pre-loaded, ₹ prefix shown
- Status: defaults to `PAID`; show optional toggle for `PARTIAL` (e.g. tenant paid ₹5,000 of ₹10,000)
- After save: haptic success + toast + sheet closes + `queryClient.invalidateQueries(['payments', 'dashboard'])`
...
─────────────────────────────────────────
  ...
  ─── Management ──────────────────────
  👥 Tenants                           →
  📊 Reports  (❌ no API yet — hide or grey out)
...
## 19. Data Model — Property → Floor → Unit → Slot

The backend has a 4-level hierarchy. Understanding this is critical before building anything.

```
Property (PG / HOSTEL / APARTMENT)
  └── Floor (floor_number: -2 to N; -1/-2 = basement, 0 = ground, 1 = first, ...)
        └── Unit  (a physical room — unit_number e.g. "101", capacity = how many beds)
              └── Slot  (the actual occupancy unit — monthly_rent lives here)
                    └── Tenant (one active tenant per slot at a time)
```

**What this means in practice:**

| Data model level | What it maps to in the real world |
|---|---|
| Property | The whole building (e.g. "Sunrise PG") |
| Floor | A floor in the building (Ground Floor, First Floor, Basement) |
| Unit | A room door — Room 101, Room 202 |
| Slot | A bed/space inside that room. PG single room = 1 slot. PG sharing = 2+ slots per unit |
| Tenant | The person renting a specific slot |

**Key API consequence:**
- `monthly_rent` is on **Slot**, not on Property or Tenant
- You can't get "total expected rent for a property" without summing across its slots — the `/api/dashboard/` endpoint does this for you
- To build the room grid: use `GET /api/slots/?property_id={id}` — this flat endpoint returns all slots with their floor, unit, occupancy, and active tenant pre-joined

**Property types and their slot types:**
- `PG` → slots are `ROOM` type (single bed per slot typically)
- `HOSTEL` → slots are `BED` type (dorm-style, multiple beds per unit)
- `APARTMENT` → slots are `ROOM` type (whole-room rental)

**Floor numbering:**
- `floor_number: 0` = Ground Floor
- `floor_number: 1` = First Floor (and so on)
- `floor_number: -1` = First Basement
- `floor_number: -2` = Second Basement
- Display logic: show "Ground" for 0, "Floor N" for positive, "Basement" for negative

**Tenant lifecycle:**
1. Assign to slot: `POST /api/tenants/` with `slot_id`
2. Active while: `is_active = true` (no exit_date)
3. Exit: `POST /api/tenants/{id}/exit/` with `exit_date` — frees the slot for next tenant
4. Slot shows `is_occupied: false` immediately after exit

---

## 20. API Map — What Each Screen Actually Fetches

This is the ground truth. Only build features that have API backing.

### Home Screen
```
GET /api/dashboard/                    → summary stats, current month, properties list, recent payments
GET /api/tenants/?unpaid=true          → "Needs Attention" overdue tenants (with phone numbers)
```

### Properties List
```
GET /api/properties/                   → property cards (name, type, address, slug)
GET /api/dashboard/                    → occupancy + collected rent per property (from properties[])
```

### Property Detail
```
GET /api/properties/by-slug/{slug}/    → property meta (name, type, address)
GET /api/slots/?property_id={id}       → ALL slots with floor/unit/occupancy/active_tenant — builds room grid
GET /api/dashboard/?property_id={id}  → property-scoped stats strip (occupancy%, floors count, rent)
GET /api/tenants/?property_id={id}&active=true  → Tenants tab
GET /api/payments/?property_id={id}   → Payments tab
```
> Note: `/api/floors/` and `/api/units/` are used when ADDING/EDITING floors and units, not for the room grid view.

### Payments List
```
GET /api/payments/?month=X&year=Y      → paid/partial/pending payment records
GET /api/tenants/?unpaid=true&month=X&year=Y  → tenants with NO payment record (UNPAID section)
```

### Record Payment (bottom sheet)
```
GET /api/tenants/?active=true          → tenant search/select dropdown
POST /api/payments/                    → create payment record
```

### Tenants List
```
GET /api/tenants/?query=X&property_id=Y&active=Z&unpaid=W   → filtered tenant list
```

### Add Tenant
```
GET /api/properties/                   → property select
GET /api/slots/?property_id={id}&vacant=true   → vacant slot select
POST /api/tenants/                     → create tenant (requires slot_id)
```

### Tenant Detail
```
GET /api/tenants/{slug}/               → tenant profile (use by-slug or id)
GET /api/payments/?tenant_id={id}      → payment history timeline
POST /api/payments/                    → record payment pre-filled with this tenant
POST /api/tenants/{id}/exit/           → mark tenant as exited
```

### Add / Edit Property
```
POST /api/properties/                  → create
PUT  /api/properties/by-slug/{slug}/   → update
DELETE /api/properties/by-slug/{slug}/ → delete
```

### Add Floor (bottom sheet)
```
POST /api/properties/{id}/floors/      → create floor
```

### Onboarding Setup (bottom sheet)
```
POST /api/properties/                  → step 2: create property
POST /api/properties/{id}/floors/      → step 3: create N floors
POST /api/properties/{id}/floors/{id}/units/   → step 4: create units per floor
POST /api/properties/{id}/floors/{id}/units/{id}/slots/  → step 4: create slots per unit
```

### Settings / Profile
```
GET  /api/auth/me                      → user name, email, member since
PATCH /api/auth/me                     → update name
POST /api/auth/logout                  → logout (send refresh token in body for mobile)
POST /api/auth/refresh                 → token refresh (send refresh in body)
```

---

## 21. Features — What Exists vs. What Doesn't

| Feature | Backed by API? | Notes |
|---|---|---|
| Login / Signup / Forgot Password | ✅ | Full auth endpoints |
| Dashboard overview stats | ✅ | `/api/dashboard/` |
| "Needs Attention" unpaid tenants | ✅ | `/api/tenants/?unpaid=true` includes phone |
| Tap-to-call from home screen | ✅ | Phone is in tenant list response |
| Properties list with occupancy | ✅ | Dashboard `properties[]` has `occupied`/`vacant` |
| Room grid on property detail | ✅ | `/api/slots/?property_id=X` flat endpoint |
| Visual floor grouping in room grid | ✅ | Each slot has `floor_number` + `floor_name` |
| Vacant room → Assign tenant | ✅ | POST /api/tenants/ with `slot_id` |
| Payment month navigator | ✅ | `?month=X&year=Y` filter exists |
| Grouped UNPAID / PAID sections | ✅ | Two calls merged in UI |
| Swipe-to-record payment | ✅ | POST /api/payments/ creates new record |
| Payment methods: UPI/Cash/Bank/Cheque | ✅ | `CASH`, `UPI`, `BANK_TRANSFER`, `CHEQUE`, `OTHER` |
| Partial payment status | ✅ | `payment_status: PARTIAL` |
| Tenant payment history | ✅ | `/api/payments/?tenant_id=X` |
| Tenant exit flow | ✅ | `POST /api/tenants/{id}/exit/` |
| Month navigator (← →) | ✅ | Client-side state + query params |
| Add Floor / Unit / Slot | ✅ | Nested endpoints exist |
| Search tenants by name/phone | ✅ | `?query=X` |
| Filter payments by property/month | ✅ | All filter params exist |
| Profile edit (name only) | ✅ | PATCH /api/auth/me |
| Token refresh on 401 | ✅ | POST /api/auth/refresh (refresh in body for mobile) |
| **Reports / Export** | ❌ | No API endpoint — do not build |
| **Push notifications** | ❌ | No backend support — do not build |
| **In-app messaging** | ❌ | No API endpoint |
| **Payment reminders (auto)** | ❌ | No backend support |
| **Property-level photos** | ❌ | No image upload endpoint |
| **Tenant photo** | ⚠️ | `photo_url` field exists (string URL) but no upload endpoint — accept URL input only |
